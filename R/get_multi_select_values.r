...
#' @return vector
#'
#' @param column vector of column in data.frame (usually, after using get_notion_database, in form db$column_name )
#' @param strip_string String by which we should strip >1 selects in a single database row. Defaults to "|", as this is the default behaviour in get_notion_database().
...
#' @export
get_multi_select_values <- function(column, strip_string = "\\|", no_na = T, only_unique = T, show_progress = T){
  l1 <- length(column)
